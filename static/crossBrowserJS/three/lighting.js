...
var LightingTest = function() {
  ...
  function run(canvas, cb, anti, ID) {
    ...
    // var spikearray = new Uint8Array(gl.drawingBufferWidth * gl.drawingBufferHeight * 4);
    // gl.readPixels(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight, gl.RGBA, gl.UNSIGNED_BYTE, spikearray);
    // resstr = "";
    // for(var i=0;i<spikearray.length/4;i++){
    //     if(spikearray[4*i] == 0 && spikearray[4*i + 1] == 0 && spikearray[4*i + 2] ==0 && spikearray[4*i + 3] == 255){
    //
    //     }else{
    //       resstr += String.fromCharCode(spikearray[4*i], spikearray[4*i+1], spikearray[4*i+2], spikearray[4*i+3]);
    //     }
    // }
    // spikeglobal += calcSHA1(resstr);
  };
  ...
  function init() {
    ...
    var texture = textureLoader.load("/static/crossBrowserJS/three/textures/color.png");
