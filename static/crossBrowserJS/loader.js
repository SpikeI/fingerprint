...
(function() {
  ...
  var createCopyButton, mobileAndTabletCheck, root;
  fingerprint_array = [];
  ...
  Loader = (function() {
    ...
    Loader.prototype.assetLoaded = function(parent_resolve) {
      this.numLoaded++;
      if (this.numLoaded === this.numberOfAssets) {
        return this.beginTests(parent_resolve);
      }
    };
    ...
    Loader.prototype.beginTests = function(parent_resolve) {
      ...
      postProgress = (function(_this) {
        return function() {
          ...
          if (_this.numComplete === _this.numberOfTests) {
            ...
            var res = sender.sendData(parent_resolve);
            // parent_resolve()
            return res;
          }
        };
      ...
      Tester = (function() {
        function Tester(testList, dest) {
          ...
          this.canvas = $("<canvas id=\"spike\" width='" + d + "' height='" + d + "'/>").appendTo(dest)[0];
          ...
          testDone = (function(_this) {
            return function() {
              ...
              if (_this.numTestsComplete < _this.testList.length) {
                var spike =  _this.testList[_this.numTestsComplete].begin(_this.canvas, testDone);

                // spike work
                let mycanvas = $("#spike")[0];
                let gl = getGL(mycanvas);
                let spikearray = new Uint8Array(gl.drawingBufferWidth * gl.drawingBufferHeight * 4);
                gl.readPixels(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight, gl.RGBA, gl.UNSIGNED_BYTE, spikearray);
                resstr = "";
                for(let i=0;i<spikearray.length/4;i++){
                ...
                console.log("********************************************");
                console.log(_this.testList[_this.numTestsComplete]);
                console.log(spikearray);
                fingerprint_array.push(calcSHA1(resstr));
                console.log("********************************************");
                // spikeglobal = "";
